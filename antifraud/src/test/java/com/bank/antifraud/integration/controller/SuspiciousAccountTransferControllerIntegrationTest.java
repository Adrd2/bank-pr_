...
import com.bank.antifraud.controller.SuspiciousAccountTransferController;
...
class SuspiciousAccountTransferControllerIntegrationTest extends TestContainer {
    @Autowired
    ...
    @BeforeAll
    static void init() {
        dto = new SuspiciousAccountTransferDto(3L, 3L
                , false, false
    ...
    void readByIdPositiveTest() throws Exception {
        mockMvc.perform(get("/suspicious/account/transfer/4"))
    ...
    void readAllPositiveTest() throws Exception {
        mockMvc.perform(get("/suspicious/account/transfer?ids=4,2"))
                .andExpect(status().isOk())
    ...
    void updatePositiveTest() throws Exception {
        var request = put("/suspicious/account/transfer/4")
                .content(objectMapper.writeValueAsString(dto))
                .contentType(MediaType.APPLICATION_JSON);
        mockMvc.perform(request)
                .andExpect(status().isOk())
